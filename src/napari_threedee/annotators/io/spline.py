...
def validate_spline_layer(layer: napari.layers.Points):
    ...
    if SplineAnnotator.SPLINE_ID_COLUMN not in layer.features:
        error = f"{SplineAnnotator.SPLINE_ID_COLUMN} not in layer features."
...
def validate_spline_zarr(n3d_zarr: zarr.Array):
    ...
    if "annotation_type" not in n3d_zarr.attrs:
        raise ValueError("cannot read as n3d spline zarr.")
    elif n3d_zarr.attrs["annotation_type"] != SplineAnnotator.ANNOTATION_TYPE:
        raise ValueError("cannot read as n3d spline zarr.")
    elif SplineAnnotator.SPLINE_ID_COLUMN not in n3d_zarr.attrs:
        raise ValueError(f"{SplineAnnotator.SPLINE_ID_COLUMN} not found.")
...
def layer_to_zarr(layer: napari.layers.Points) -> zarr.Array:
    ...
    n3d.attrs["annotation_type"] = SplineAnnotator.ANNOTATION_TYPE
    spline_id_key = SplineAnnotator.SPLINE_ID_COLUMN
...
def zarr_to_layer_data_tuple(n3d_zarr: zarr.Array) -> LayerDataTuple:
    ...
    spline_id = n3d_zarr.attrs[SplineAnnotator.SPLINE_ID_COLUMN]
    spline_order = n3d_zarr.attrs[SplineAnnotator.SPLINE_ORDER_KEY]
    layer_kwargs = {
        "features": {SplineAnnotator.SPLINE_ID_COLUMN: spline_id},
